...
;(function($, undefined) {
	...
	function Modal(options) {
		...
		this.$modal = $(this.options.target).attr('class', 'modal').hide();
		var self = this;
		function init() {
	...
	$.extend(Modal.prototype, {
		show: function() {
			...
			if (!this.$modal.length) {
				this.$modal = $('<div class="modal " id="' + this.options.target.substr(1) + '" tabindex="-1" role="dialog" aria-hidden="true"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="btn-close" data-dismiss="modal" aria-hidden="true">&times;</button><h4 class="modal-title">&nbsp;</h4></div><div class="inner"/></div></div></div>').appendTo(this.options.appendTo).hide();
			}

			this.$modal.find('.modal-header h4').html(this.options.title);

			if (this.options.cssClass !== undefined) {
				this.$modal.attr('class', 'modal  ' + this.options.cssClass);
			}
