...
namespace AIText.Controllers
{
    public class PaintAccountController : BaseController
    {
        ...
        public async Task<IActionResult> DoDraw(string id)
        {
            var model = Db.Queryable<PaintAccount>().Where(o => o.Id == id).First();
            if (model != null && model.Site?.Contains("liblib") == true)
            {
                string keyword = "dirndl rental munich";
                var content = await Liblibai.Text2img(model.AccessKey, model.SecretKey, $"高清产品图，{keyword}");
                if (content.Contains("|"))
                {
                    return Ok("生成图片出错" + content.Substring(content.IndexOf("|")));
                }
                else
                {
                    var result = await Liblibai.status(model.AccessKey, model.SecretKey, content);
                    return PartialView("DoDraw", result);
                }

            }
            return PartialView("DoDraw","https://liblibai-tmp-image.liblib.cloud/img/26e88a6216dd4830b3c0f5f2d89ea549/9c1e1eeef4ea28a9f1cf64f91f1455c3aa50ddd89a1955c2d819e6f2d064e9d6.png");
        }
    }
}
